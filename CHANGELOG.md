# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.2.4] - 2025-01-12

### Changed
- README: Show full download URL for transparency

## [0.2.3] - 2025-01-12

### Added
- README: Direct download link to latest skill file

## [0.2.2] - 2025-01-12

### Added
- README: Development section with versioning instructions

## [0.2.1] - 2025-01-12

### Added
- Subagent strategies for unbiased self-audit
- README: Relationship to paper section explaining qualitative interpretation

### Changed
- Streamlined provenance section with links to theory.md and README

## [0.2.0] - 2025-01-12

### Added
- Supporting evidence section in output format (source, URL, snippet)
- Evidence documentation guidelines for grounded claims

## [0.1.3] - 2025-01-12

### Fixed
- Frontmatter now conforms to agentskills.io spec
- Version moved to metadata field
- Added license field
- Shortened description

## [0.1.2] - 2025-01-12

### Added
- Repository and release links in SKILL.md
- Version check API endpoint for update detection
- Instructions for checking and offering updates to users

## [0.1.1] - 2025-01-12

### Added
- GitHub Action to build .skill packages on release

## [0.1.0] - 2025-01-12

### Added
- Initial release
- Core audit protocol with three dimensions: source fidelity, information delta, rhetorical fairness
- Claim taxonomy: factual, interpretive, synthetic, editorial
- Salience classification: load-bearing, supporting, illustrative
- Nine verdict types including INSUFFICIENT and UNVERIFIABLE
- Confidence calibration based on ISR thresholds
- Guidance for inaccessible sources
- Resource files:
  - `theory.md` - Information-theoretic framework with EDFL, ISR, and empirical findings
  - `failure-modes.md` - 12 common failure patterns
  - `remediation.md` - Strategies and decision tree
  - `document-audit.md` - Protocol for complete documents

### Credits
- Based on [Predictable Compression Failures](https://arxiv.org/abs/2509.11208) by Chlon, Karim, & Chlon (Hassana Labs)
- Adapted from [Strawberry MCP](https://github.com/leochlon/pythea/tree/main/strawberry)
